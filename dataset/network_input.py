...
Input tensor channel layout (17 channels total):
    ── Spectral bands (raw, /10000 normalised) ──────────────────────────
    ...
    7: B9  Cirrus           (1.36–1.38 µm)  — zero-filled if absent in source
    ── Precomputed derived features (percentile-normalised) ─────────────
    8:  RGB_R   (B4 percentile-normalised)
    9:  RGB_G   (B3 percentile-normalised)
    10: RGB_B   (B2 percentile-normalised)
    11: HSV_H   Hue ∈ [0,1]
    12: HSV_S   Saturation ∈ [0,1]
    13: HSV_V   Value ∈ [0,1]
    14: Sobel_X  (on luminance of RGB)
    15: Sobel_Y
    16: Sobel_Magnitude
"""
...
N_SPECTRAL = 8   # channels 0–7
N_DERIVED  = 9   # channels 8–16


# ── On-the-fly spectral index functions ────────────────────────────────
...
def inp_all_cirrus(inp_img):
    """All 8 spectral bands: B1–B7 + B9."""
    return inp_img[:, 0:8, :, :]


def inp_rgb(inp_img):
    """Spectral RGB: B2, B3, B4 (3 channels)."""
...
def inp_swirndsi(inp_img):
    """SWIR + NDSI: B2–B7 + NDSI (7 channels)."""
...
def inp_swirndsi_ndwi(inp_img):
    """SWIR + NDSI + NDWI: B2–B7 + NDSI + NDWI (8 channels)."""
    bands = inp_img[:, (1, 2, 3, 4, 5, 6), :, :]
    ndsi  = compute_NDSI(inp_img)
...
def inp_swirndwi(inp_img):
    """SWIR + NDWI: B2–B7 + NDWI (7 channels)."""
    bands = inp_img[:, (1, 2, 3, 4, 5, 6), :, :]
    ndwi  = compute_NDWI(inp_img)
...
def inp_cirrus_ndsi(inp_img):
    """Cirrus + SWIR + NDSI: B2–B7, B9 + NDSI (8 channels)."""
...
def inp_rgb_precomp(inp_img):
    """Precomputed percentile-normalised RGB (3 channels)."""
    return inp_img[:, 8:11, :, :]


def inp_hsv(inp_img):
    """Precomputed HSV (3 channels)."""
    return inp_img[:, 11:14, :, :]


def inp_sobel(inp_img):
    """Precomputed Sobel X, Y, Magnitude (3 channels)."""
    return inp_img[:, 14:17, :, :]


def inp_rgb_hsv(inp_img):
    """Precomputed RGB + HSV (6 channels)."""
    return inp_img[:, 8:14, :, :]


def inp_swir_sobel(inp_img):
    """SWIR bands + Sobel (9 channels): B2–B7 + Sobel_X/Y/Mag."""
    bands = inp_img[:, (1, 2, 3, 4, 5, 6), :, :]
    sobel = inp_img[:, 14:17, :, :]
    return torch.cat([bands, sobel], dim=1)


def inp_swirndsi_sobel(inp_img):
    """SWIR + NDSI + Sobel (10 channels): B2–B7 + NDSI + Sobel_X/Y/Mag."""
    bands = inp_img[:, (1, 2, 3, 4, 5, 6), :, :]
    ndsi  = compute_NDSI(inp_img)
    sobel = inp_img[:, 14:17, :, :]
    return torch.cat([bands, ndsi, sobel], dim=1)


def inp_all_derived(inp_img):
    """All 17 channels: spectral(8) + RGB(3) + HSV(3) + Sobel(3)."""
    return inp_img[:, :17, :, :]
...
class CustomInput:
    ...
    def __init__(self, bands=None, indices=None):
        ...
        self.__name__ = (
            f'custom_{"_".join(self.band_names)}_{"_".join(self.index_names)}')
...
def get_inp_func(mode, bands=None, indices=None):
    ...
    bands : list of str, optional
        Band/feature names for custom mode.
    indices : list of str, optional
        Spectral index names for custom mode (e.g. ['NDSI', 'NDWI']).

    Returns
    -------
    callable
        Function mapping (B, 17, H, W) → (B, C_selected, H, W).
    """
    if mode == 'custom':
...
def get_inp_channels(mode, bands=None, indices=None):
    """Return the number of input channels for a given mode."""
    if mode == 'custom':
...
def list_available_modes():
    ...
    for name, (fn, n_ch) in _PRESET_MODES.items():
        print(f'  {name:22s}  →  {n_ch} channels')

    print('\n=== On-the-fly Spectral Indices ===')
    for name in INDEX_REGISTRY:
        print(f'  {name}')

    print('\n=== Available Bands / Derived Features ===')
    seen = set()
